...
function Register() {
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    ...
    try {
      setLoading(true);
      const { successfully, state, msg }: any = await RegisterApi({
        password,
        fullname,
        username,
        email,
      });
      console.log(state, successfully, msg);
      const credential = username;
      if (state) {
        const { status, user } = await LoginApi({ credential, password });
          ...
          {showModal ? (
            <div className="absolute w-full h-full bg-grayOne/90 z-40 flex justify-center items-center">
              <Successfully
                ...
                contentButton={success ? "Página Inicial" : "OK"}
                content={
